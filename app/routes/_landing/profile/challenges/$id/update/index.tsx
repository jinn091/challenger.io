...
export async function loader({
	request,
	params
}: LoaderFunctionArgs): Promise<TypedResponse<Challenge | null>> {
...
> {
	...
	const createChallengeResult = await updateChallengeById({
		id: challengeId,
		name,
		targetLink: target,
		prize,
	...
	if (!createChallengeResult.ok) {
		return json({
			...
			error: {
				fields,
				message:
					"Something went wrong while updating challenge. Try again later :)"
			}
		});
	}
...
export default function ProfileRoute(): React.JSX.Element {
	...
	const [selectedMethods, setSelectedMethods] = useState<WebHackingMethods[]>(
		data?.methods as WebHackingMethods[]
	);
	...
	return (
		<Form method="POST" className="flex flex-col gap-4">
			...
			{actionData?.ok && (
				<p className="success">The challenge has been updated!</p>
			)}
